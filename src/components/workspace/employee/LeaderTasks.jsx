...
const EMPTY = { title: '', description: '', assigned_to: '', priority: 'medium', due_date: '', status: 'todo' }
...
export default function LeaderTasks({ employee }) {
  ...
  useEffect(() => { if (supabase && employee.brand_id) load() }, [])
  ...
  const save = async () => {
    ...
    if (editing === 'new') {
      ...
      if (!error) await logActivity(employee.id, `Created task: ${form.title}`, 'task', null, { brand_id: employee.brand_id })
    } else {
  ...
  return (
    <div>
      {/* STATS */}
      <div className="ws-stats" style={{ marginBottom: 24 }}>
      ...
      {teamMembers.length > 0 && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {teamMembers.map(m => {
            ...
            return (
              <div key={m.id} onClick={() => setFilter(f => ({ ...f, member: f.member === m.id ? '' : m.id }))}
                ...
                <div>
                  ...
                  <div style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 8, color: activeTasks > 0 ? '#F59E0B' : '#10B981' }}>{activeTasks} active task{activeTasks !== 1 ? 's' : ''}</div>
                </div>
              </div>
            )
          })}
                ...
                <div key={t.id} className="ws-task" onClick={() => openTask(t)} style={{ borderColor: selected?.id === t.id ? 'rgba(0,200,255,0.4)' : undefined }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                      <button onClick={e => { e.stopPropagation(); setForm({ title: t.title, description: t.description || '', assigned_to: t.assigned_to || '', priority: t.priority, due_date: t.due_date || '', status: t.status }); setEditing(t) }}
                  ...
                  <div className="ws-task-meta">
                    ...
                    {t.created_by && <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 8, color: 'rgba(240,244,255,0.2)' }}>by {t.created_by}</span>}
                  </div>
                </div>
              )
            })}
          ...
          <div className="ws-modal">
            ...
            <div className="ws-form-full">
              ...
              <select className="ws-input" value={form.assigned_to} onChange={e => setForm({ ...form, assigned_to: e.target.value })}>
                ...
                {teamMembers.map(m => <option key={m.id} value={m.id}>{m.full_name}{m.role ? ` — ${m.role}` : ''}</option>)}
              </select>
            </div>
            ...
            <div className="ws-modal-actions">
              ...
              <button className="btn-primary" onClick={save}>
                {editing === 'new' ? 'Assign Task' : 'Save Changes'}
              </button>
            </div>
          </div>
        </div>
